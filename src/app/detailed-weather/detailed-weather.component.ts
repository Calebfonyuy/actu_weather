...
export class DetailedWeatherComponent implements OnInit {
  ...
  onHourDisplay(){
    console.log('normally hour is displayed')
  ...
  onDayDisplay() {
    console.log('normally day is displayed')
   ...
   this.http.get(req ,{responseType:'json', params: this.param_weather}).subscribe((res:any) => {
      if(request_type == "forecast"){
        for (var i=0; i < res.cnt; i++ ){
          ...
          console.log(this.forecast_data.length)
        }
      }
    });
    console.log('end of get data', this.forecast_data.length)
  }
  get_namedate(dt_txt){
   ...
   fill_data(forecast_data){
     console.log('forecast size = ',forecast_data.length)
     console.log('printing forcast ',forecast_data)
     for(var i=0;i<8; i++){
       
         let hourly_data_temp = new current_data();

         if(i==0){
           hourly_data_temp.name = "Auj. "+this.get_heure(forecast_data[i].dt) ;
         }
         else if((i!=0)&&(this.get_namedate(forecast_data[i-1].dt)!=this.get_namedate(forecast_data[i].dt))){
           hourly_data_temp.name = "Demain "+this.get_heure(forecast_data[i].dt) ;
         }else{
